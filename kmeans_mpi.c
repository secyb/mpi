...
int  main(int argc, char *argv[])
{
	...
	if (!rank) {  //进程0产生随机中心点
		...
		for (i = 0; i<K; i++)
			for (j = 0; j<D; j++)
				cluster_center[i][j] = data[(int)((double)800 / (i + 1.0))][j];
	...
	//printf("process time is %9.7f\n", diff);
	//printf("The time used %f ms by clock()\n",difftime(c_end,c_start)); 
	int end = time((time_t*)NULL); 
	printf("执行了%d秒",end-begin); 
}
...
float **loadData(int *k, int *d, int *n)
{
	...
	float **arraydata;
	FILE *fp;
	if ((fp = fopen("/home/data.txt", "r")) == NULL)    fprintf(stderr, "cannot open data.txt!\n");
